...
const SkillSharing = ({ marketplace, nft }) => {
  ...
  const loadCollections = async () => {
    ...
    for (let i = 1; i <= itemCount; i++) {
      ...
      if (!item.sold && item.category === "Skill Sharing") {
  ...
  if (collections.length === 0) {
    return (
      <h2 style={{ marginTop: "20px", display: "flex", justifyContent: "center" }}>
    ...
    return collections.map((collection) => {
      ...
      return (
        <div className="flex justify-center" key={collection.name}>
          <Col>
            <Card>
              <Card.Img variant="top" className="img-size" src={firstItem.image} />
              <Card.Body>
